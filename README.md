...
<p align="center">
  <sub>v1.4.0 · Windows 10/11 · Marvis / QClaw 已测试 · 更多 Agent 适配中</sub>
</p>
...
<p align="center">
  ...
  ▸ <strong>Steam 管理</strong>：伪造 ACF 版本信息 + 自动抓取 SteamDB 最新 BuildID/Manifest GID，一键版本同步 + 商店资源下载<br />
...
> 当前 v1.4.0 已在 Marvis / QClaw 平台完成测试。其他 Agent 平台的理论支持见下方说明。
...
Steam 版本修复是一个**从检测到锁定的一键流程**。Agent 收到以下任一请求时会执行完整链路：

| 你想做什么 | 直接说 |
|-----------|--------|
| **全面版本修复（推荐）** | 「帮我检查/更新/修复一下无限暖暖的 Steam 版本」 |
| 查看 Steam 状态 | 「Steam 版本状态怎么样」「看看 ACF」 |
| 清理空间 | 「帮我清理一下 Steam 目录空间」「骨架化」 |
...
Steam 版本修复是一个连贯的线性流程：**检测 → 对比 → 更新 → 锁定**，用户无需分步操作。
...
> **数据保存策略**：每次查询时，Agent 自动将当前页面所有探索数据保存到 `{USER_DATA_DIR}nuan_explore.json`。缓存有效期 7 天，可随定时任务定期更新。
...
> **消费历史持久化**：每次氪条查询结果自动追加到 `{USER_DATA_DIR}nuan_recharge_history.json`，只增补不覆写，可追溯历史查询记录。
...
查询套装抽卡统计（数据来源：`{USER_DATA_DIR}nuan_gacha_stats.json`）：
...
| 7 | 奇想手账 Cookie 过期时需重新登录，更新 `{USER_DATA_DIR}nuan_profile.json` |
| 8 | **抽卡记录首次爬取较慢**：第一次爬取需要遍历所有历史抽卡页面，预计 5-15 分钟。爬取完成后数据保存到 `{USER_DATA_DIR}nuan_gacha_stats.json`，后续查询直接读取本地，无需重新爬取 |
| 9 | **首次使用需手动登录**：`{USER_DATA_DIR}nuan_profile.json` 不存在时，Agent 会打开可见 Chrome 引导您完成奇想手账登录（含验证码），登录后自动提取 Cookie 持久化 |
| 10 | **定时任务推荐**：创建自动任务时（如每月更新抽卡记录），可一并更新探索进度数据（`{USER_DATA_DIR}nuan_explore.json`），保持本地数据最新 |

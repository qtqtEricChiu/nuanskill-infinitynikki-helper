...
<p align="center">
  <em>Steam 版本管理（ACF 反更新 · SteamDB 自动检测 · 骨架化清理）+ 奇想手账监控（体力 · 美鸭梨 · 抽卡统计 · 探索进度查询）</em>
</p>

<p align="center">
  <sub>v1.1.0 · Windows 10/11 · Marvis / QClaw 已测试 · 更多 Agent 适配中</sub>
</p>
...
> 当前 v1.1.0 已在 Marvis / QClaw 平台完成测试。其他 Agent 平台的理论支持见下方说明。
...
通过 browser agent 访问 `https://myl.nuanpaper.com/tools/journal`，自动完成：
...
通过 browser agent 访问奇想手账「探索总览」标签页（第 2 个标签），提取各区域探索数据：
...
| 7 | 奇想手账 Cookie 过期时需重新登录，更新 `user-data/nuan_user_profile.json` |
| 8 | **抽卡记录首次爬取较慢**：第一次爬取需要遍历所有历史抽卡页面，预计 5-15 分钟。爬取完成后数据保存到 `user-data/nuan_gacha_stats.json`，后续查询直接读取本地，无需重新爬取 |
| 9 | **首次使用需手动登录**：`nuan_user_profile.json` 不存在时，Agent 会打开可见 Chrome 引导您完成奇想手账登录（含验证码），登录后自动提取 Cookie 持久化 |
...
- ✅ 新增「自动任务推荐」：引导创建定时任务时一并更新相关数据
- ✅ QClaw 平台测试通过（支持本地导入或从 Github 导入）

### v1.0.1（2026-06-26）

- ✅ **新增「首次初始化」工作流**：首次使用或 Cookie 缺失时，自动弹出可见 Chrome 引导用户手动登录，完成后提取 Cookie 持久化
- ✅ 新增第 9 条强制规则：首次初始化必须可见 Chrome（第 7 条规则的唯一例外）
- ✅ 统一 Chrome 用户数据目录为 `Cache\chrome_temp_profile\`，SteamDB 与奇想手账共用
- ✅ 意图路由表新增「首次初始化 / Cookie 过期」触发行
- ✅ 关联输出例外：日活/体力/美鸭梨三项每日任务可一口气给出

### v1.0.0（2026-06-25）— 正式发布

- ✅ 整合 Steam 管理与奇想手账两大能力集
- ✅ SKILL.md 采用 Agent Skill 标准格式（YAML front matter + 强制规则 + 意图路由表）
- ✅ 脚本统一 `nuanskill-` 命名前缀
- ✅ 单文件化：配置内嵌到 `nuanskill-infi-manager.ps1`，删除 `config.json` 和 `infi_steamdb_fetch.py`
- ✅ 8 条不可违反的执行规则（时间计算、昵称映射、数据源优先级等）
- ✅ Marvis 平台测试通过
- 📋 待完善：OpenClaw / WorkBuddy / QClaw 平台测试

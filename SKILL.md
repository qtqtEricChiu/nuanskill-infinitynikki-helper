...
本技能为《无限暖暖》(AppID: 3164330) 提供以下能力集：

| 能力集 | 数据源 | 核心动作 |
|--------|--------|---------|
| **Steam 管理** | 本地 ACF / SteamDB | 版本检测、ACF 更新、骨架化清理、资源下载、网络诊断 |
...
> 4. **`references/nuanskill-browser.md`** → 了解浏览器操作（Chrome 启动、CDP、CF 验证等通用技能）
> 5. 触发具体任务时 → 前往对应 `references/*.md` 读取完整执行流程
> 6. **卡住了？** → 查阅 `references/nuanskill-known-issues.md`，按能力标签查找对应踩坑
...
浏览器操作通用技能（Chrome 启动、CDP 连接、CF 验证、Cookie 提取）见：
...
| 通用 | 浏览器初始化 / CDP 连接 / CF 验证释放 | `references/nuanskill-browser.md` | 「浏览器」「Chrome」「CDP」「有头」「可见」「验证」「CF」 |
| 能力① Steam 管理 | **全面版本修复**（检测→更新→锁定一键流程） | `references/nuanskill-steamdb-check.md` + 浏览器通用参考 + 执行脚本 | 「版本」「更新」「SteamDB」「ACF」「检测」「修复」「Steam 状态」「查看」 |
| 能力① Steam 管理 | **商店资源下载**（capsule/bg/logo 等资产打包） | `references/nuanskill-steamdb-assets.md` + 浏览器通用参考 | 「资源」「下载」「封面」「资产」「asset」「capsule」「logo」「banner」「提取图片」 |
| 能力① Steam 管理 | 空间清理 / 还原 | `references/nuanskill-steamdb-check.md` + 执行 `scripts\nuanskill-infi-manager.ps1` | 「骨架化」「骨骼化」「清理空间」「还原 X6Game」「释放空间」 |
| 能力① Steam 管理 | 网络诊断 | `scripts\nuanskill-infi-manager.ps1 steamdb-check`（失败时自动触发） | 「连不上」「网络」「SteamDB 打不开」 |
...
├── references/
│   ├── nuanskill-browser.md                  # 浏览器操作通用参考（Chrome/CDP/CF/Cookie）
...
│   ├── nuanskill-steamdb-assets.md            # Steam 商店资源下载（能力①子技能）
...
当前版本：**v1.4.0（2026-06-28）**。查看所有历史版本请参阅 `CHANGELOG.md`。

...
## 11. 昵称→正式名映射 — 视觉误导与搜索精度不足

**现象**：用户说「小红帽套」时，Agent 因看到「红色 + 帽子」的视觉印象，错误指向了「绯夜狂想录」（玩偶套），而正确的映射是「她是不驯火」。

**根因**：
- 先入为主的视觉判断（红色 + 帽子 = 小红帽），没有深入核对搜索结果
- 搜索结果被无关内容干扰后，没有换更精准的关键词继续深挖
- 没有拿猜测的正式名做正向交叉验证
...
```
用户输入昵称 →
1. 搜「昵称 + 无限暖暖 + 套装」→ 排除杂鱼干扰
2. 若结果模糊 → 换「昵称 + 五星」「昵称 + 无限暖暖」再搜
3. 有候选正式名后 → 搜「候选名 + 昵称」交叉验证是否匹配
4. 优先采信高权威来源（官网文章、正规游戏媒体），不要凭视觉印象做直觉判断
5. 多条来源一致确认后再返回结果
```
...
| 昵称 | 容易误判 | 正确映射 | 关键线索 |
|------|---------|---------|---------|
| 小红帽 | 绯夜狂想录（玩偶套，红色+帽子） | 她是不驯火 | 官方文章写明灵感来源于「小红帽」
